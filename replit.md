...
## Recent Changes
- July 31, 2025: Enhanced Dashboard and Learn page functionality: implemented property search functionality with address and city filtering, updated "View All Properties" button to scroll to top when navigating to Invest page, changed placeholder text to "Search for property" for better user clarity, removed all duplicate close buttons from popup windows (Read Article modals, tutorials, guides), and enhanced Learn page hover effects with wider button backgrounds and consistent brick red color (#A52A2A) for all interactive elements

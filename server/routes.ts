...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/property-reports", isAuthenticated, async (req, res) => {
    try {
      const validatedData = insertPropertyReportSchema.parse(req.body);
      const report = await storage.createPropertyReport({
        ...validatedData,
        businessOwnerId: (req.user as any).id,
        isPublished: true,
      ...
      console.error("Failed to create property report:", error);
      res.status(400).json({ error: error.message || "Failed to create property report" });
    }
  });
  
  app.get("/api/property-reports/property/:propertyId", isAuthenticated, async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Failed to fetch property reports" });
    }
  });
  
  app.get("/api/property-reports/:id/pdf", isAuthenticated, async (req, res) => {
    try {
      const reportId = parseInt(req.params.id);
      const report = await storage.getPropertyReport(reportId);
      
      if (!report) {
        return res.status(404).json({ error: "Report not found" });
      }
      
      // Simple PDF generation (you could use a proper PDF library like pdfkit)
      const pdfContent = `
40 Acres - Property Report
========================

Title: ${report.title}
Type: ${report.reportType}
Quarter/Year: ${report.quarterYear || 'N/A'}
...
---
Generated by 40 Acres Platform
      `.trim();
      
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="property-report-${reportId}.pdf"`);
      res.send(pdfContent);
    } catch (error: any) {
